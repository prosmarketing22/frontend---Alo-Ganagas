...
import '../customerLoans/CustomerLoanAccordion.css';

export const SupplierLoanAccordion = ({ onRegisterReturn, onNewLoan, refreshTrigger }) => {
  const [expandedSupplier, setExpandedSupplier] = useState(null);
  const [filters, setFilters] = useState({ search: '', debt_type: '' });
  const [suppliers, setSuppliers] = useState([]);
  const [localError, setLocalError] = useState(null);

  const { getGroupedBySupplier, loading } = useSupplierLoanApi();
  ...
  const loadGroupedData = async (searchFilters = filters) => {
    setLocalError(null);
    try {
      const data = await getGroupedBySupplier(searchFilters);
      setSuppliers(data || []);
    } catch (err) {
    ...
    return (
      <div className="loan-accordion">
        <div className="loan-accordion__loading">
          ...
          <p>Cargando préstamos de proveedores...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="loan-accordion">
      ...
      {localError && (
        <div className="loan-accordion__error">
          <span className="loan-accordion__error-icon">⚠</span>
          <span>{localError}</span>
        </div>
      )}

      {!loading && suppliers.length === 0 && !localError ? (
        <div className="loan-accordion__empty">
          <div className="loan-accordion__empty-icon">📋</div>
          <p>No hay registros de préstamos de proveedores</p>
        </div>
        ...
        <div className="loan-accordion__list">
          {suppliers.map(supplier => (
            <div
              ...
              className={`loan-accordion__item ${expandedSupplier === supplier.supplier_id ? 'loan-accordion__item--expanded' : ''}`}
            >
              <div
              ...
              >
                <div className="loan-accordion__customer-info">
                  ...
                  <div className={`loan-accordion__customer-avatar ${supplier.total_pending === 0 ? 'loan-accordion__customer-avatar--settled' : ''}`}>
                    {supplier.total_pending === 0 ? '✓' : supplier.supplier_name?.charAt(0)?.toUpperCase() || '?'}
                  </div>
                  <div className="loan-accordion__customer-details">
                    <div className="loan-accordion__customer-name-row">
                      <span className="loan-accordion__customer-name">{supplier.supplier_name}</span>
                      {supplier.total_pending === 0 && (
                        <span className="loan-accordion__settled-badge">Al día</span>
                      )}
                    </div>
                    <span className="loan-accordion__customer-phone">
                      {supplier.supplier_ruc ? `RUC: ${supplier.supplier_ruc}` : 'Sin RUC'}
                      {supplier.supplier_phone && ` | ${supplier.supplier_phone}`}
                    </span>
                ...
                <div className="loan-accordion__customer-summary">
                  {(supplier.net_by_type || []).map((item, idx) => (
                ...
                <div className="loan-accordion__customer-total">
                  {supplier.total_pending > 0 ? (
                    <div className="loan-accordion__total-breakdown">
                      {(supplier.net_by_type || []).map((item, idx) => (
                        <div key={idx} className="loan-accordion__total-item">
                          <span className="loan-accordion__total-label">{getContainerLabel(item.container_type)}:</span>
                          <span className={`loan-accordion__total-value ${item.debt_type === 'NOS_DEBEN' ? 'loan-accordion__total-value--nos-deben' : 'loan-accordion__total-value--debemos'}`}>
                            {item.debt_type === 'NOS_DEBEN' ? 'Nos debe' : 'Le debemos'} {item.pending}
                          </span>
                        </div>
                      ))}
                    </div>
                    ...
                    <>
                      ...
                      <span className="loan-accordion__total-value loan-accordion__total-value--settled">{supplier.total_loans || 0}</span>
                    </>
                  )}
                </div>

                <div className="loan-accordion__customer-actions" onClick={e => e.stopPropagation()}>
                  {supplier.pending_loans && supplier.pending_loans.length > 0 && (
                    <button
                      onClick={() => onRegisterReturn(supplier.pending_loans[0])}
                ...
                <div className="loan-accordion__content">
                  <div className="loan-accordion__section">
                    ...
                    <div className="loan-accordion__summary-grid">
                      {supplier.summary_by_type.map((item, idx) => (
